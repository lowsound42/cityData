...
var j = schedule.scheduleJob(rule, function () {
  ...
  const getNewData = resource => new Promise((resolve, reject) => {
    https.get(`https://ckan0.cf.opendata.inter.prod-toronto.ca//api/3/action/datastore_search?id=29852011-c9c2-4b6d-a79a-b718f5ed1ae2&filters={"OCCUPANCY_DATE":"${year}-${month}-${day}T00:00:00"}`, (response) => {
      let dataChunks = [];
  ...
  const getNewDataOffset = resource => new Promise((resolve, reject) => {
    https.get(`https://ckan0.cf.opendata.inter.prod-toronto.ca//api/3/action/datastore_search?id=29852011-c9c2-4b6d-a79a-b718f5ed1ae2&filters={"OCCUPANCY_DATE":"${year}-${month}-${day}T00:00:00"}&offset=100`, (response) => {
      let dataChunks = [];
      response.on("data", (chunk) => { dataChunks.push(chunk) })
        .on("end", () => {
          let data = Buffer.concat(dataChunks)
